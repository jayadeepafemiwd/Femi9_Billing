    ...
    <div class="card">
        <div class="top-bar">
            ...
            <div class="top-bar-right">
                <button class="btn btn-primary" id="mainAddBtn" onclick="handleMainAdd()">+ Add Layer</button>
            </div>
        </div>
...
let allLayers    = [];
let globalValues = []; // per-state values for global layers
let navPath      = [];
...
function currentLayer() {
    ...
    // 1. Specific layer for this exact item
    const specific = allLayers.find(l => l.parent_id === lastId);
...
function handleMainAdd() {
    ...
    pendingTags = [];
    if (!layer) { addingLayer = true; addingItem = false; }
    else         { addingItem = true; addingLayer = false; }
    render();
...
function addPendingTag(name) {
    ...
    if (!clean) return;
    const layer = currentLayer();
    // Use currentLayerNames to check saved names (handles global per-state values)
...
async function deleteItem(layerId, nameId) {
    ...
    const layer = currentLayer();
    // For global layers pass parent_name_id so only this state's value is deleted
...
function renderTopButton() {
    ...
    addBtn.textContent = layer ? `+ Add ${cap(layer.layer_type)}` : '+ Add Layer';
...
function renderCenter() {
    ...
    const layer = currentLayer();
    // Always use currentLayerNames — handles both normal and global per-state values
    const names = currentLayerNames(layer);

    let title, hint;
    if (navPath.length === 0) {
        title = layer ? cap(layer.layer_type) : 'Assign Location';
        hint  = layer ? `Click any ${layer.layer_type} to go deeper`
...
</html><?php /**PATH D:\MAMP\htdocs\femi_billing_11\resources\views/assign_location/create.blade.php ENDPATH**/ ?>