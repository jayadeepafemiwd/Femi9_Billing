    ...
    const SCHEMA = [
       ...
       // ✅ Fix:
        'type' => [
            'type'    => 'string',
            'default' => 'goods',
            'allowed' => ['goods', 'service', 'assembly_item', 'kit_item'],
        ],
        'brand_id' => [
            'type'     => 'integer',
            'default'  => null,
...
private function uploadImage($file): string
{
    try {
        ...
        if (!$src) {
            // GD fail ஆனா original save பண்ணு
            $filename = time() . '_front_' . uniqid() . '.' . $file->getClientOriginalExtension();
            $file->move($publicFolder, $filename);
            $path = 'image/product_img/' . $filename;
            $this->logProductOperation('IMAGE_UPLOAD', 'GD failed, saved original', ['path' => $path]);
            return $path;
        }
        ...
        // Compress — quality 85, reduce until < 800KB
        $filename = time() . '_front_' . uniqid() . '.jpg';
        ...
        $path = 'image/product_img/' . $filename;
        $this->logProductOperation('IMAGE_UPLOAD', 'Image compressed and saved', [
            'path'    => $path,
            'quality' => $quality,
            'size_kb' => round(strlen($imgData) / 1024, 2),
        ]);
...
public function index(Request $request)
{
    ...
    try {
        ...
        if ($request->has('item_type')) $query->where('item_type', $request->item_type);
...
// ✅ இந்த 2 lines மட்டும் add பண்ணுங்க
$settingRow = \App\Models\SettingHandle::where('category_name', 'products')->first();
$settings   = $settingRow?->config ?? [];

$executionTime = round((microtime(true) - $startTime) * 1000, 2);
// ...
return view('products.index', compact('products', 'customFields', 'settings')); // ✅ 'settings' add
...
public function edit(Product $product)
{
    ...
    return view('products.edit', compact(
        'product', 'brands', 'customFields', 'settings'
    ));
}
