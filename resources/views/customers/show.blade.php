...
<title>{{ $customer->display_name }} | Inventory</title>
<style>
...
<div class="app-shell">
  ...
  <div class="topbar-global">
    ...
    <div class="topbar-right">
      ...
      <div class="org-selector">{{ $customer->company_name ?? 'Techvolt' }} <span style="font-size:10px;">▾</span></div>
  ...
  <div class="body-layout">
    ...
    <div class="left-panel">
      ...
      <div class="customer-list">
        ...
        <div class="customer-item {{ $c->id == $customer->id ? 'active' : '' }}"
             onclick="window.location='{{ route('customers.show', $c->id) }}'">
          <div class="cb {{ $c->id == $customer->id ? 'style=background:#1a73e8;border-color:#1a73e8;' : '' }}"></div>
          <div class="customer-item-info">
            <div class="customer-item-name">{{ $c->display_name }}</div>
            <div class="customer-item-amount">₹{{ number_format($c->outstanding ?? 0, 2) }}</div>
          </div>
        </div>
        @endforeach
        ...
        <div class="customer-item active">
          ...
          <div class="customer-item-info">
            <div class="customer-item-name">{{ $customer->display_name }}</div>
    ...
    <div class="main-content">
      <div class="customer-header">
        <div class="customer-header-name">{{ $customer->display_name }}</div>
        <div class="header-actions">
          ...
          <a href="{{ route('customers.index') }}" class="btn-close-x">×</a>
        </div>
      </div>
      ...
      <div class="split-view" id="tab-overview">
        <div class="detail-left">
          ...
          <div class="contact-card">
            ...
            <div class="contact-details">
              <div class="contact-name" id="cardName">
                ...
                @if($primaryContact)<span class="primary-badge">★ Primary</span>@endif
              </div>
              <div class="contact-email" id="cardEmail">
                @if($primaryContact)
                  @if(!empty($primaryContact['email'])){{ $primaryContact['email'] }}@endif
                  @if(!empty($primaryContact['work_phone'])) &nbsp;·&nbsp; {{ $primaryContact['work_phone'] }}@endif
                  @if(!empty($primaryContact['mobile'])) &nbsp;·&nbsp; {{ $primaryContact['mobile'] }}@endif
            ...
            <div class="card-gear-wrap">
              ...
              <div class="card-gear-menu cp-gear-menu" id="cardGearMenu">
                ...
                <div class="cp-gear-menu-item" onclick="openEditCp({{ $primaryIdx }}); closeAllGears();">
                  <span>✏️</span> Edit
                </div>
                <div class="cp-gear-menu-sep"></div>
                <div class="cp-gear-menu-item danger" onclick="openDelConfirm({{ $primaryIdx }}); closeAllGears();">
                  <span>🗑️</span> Delete
                </div>
                @else
            ...
            <div class="section-body" id="body-address">
              ...
              <div class="address-block">
                <div style="display:flex;align-items:center;justify-content:space-between;">
                  ...
                  @if($hasBilling)<button onclick="openAddressPanel('billing');return false;" style="background:none;border:none;cursor:pointer;color:#888;font-size:13px;padding:2px 6px;">✏️</button>@endif
                </div>
                <div class="address-value">
                  @if($hasBilling)
                    {{ $billing['attention'] ?? '' }}<br>{{ $billing['street1'] ?? '' }} {{ $billing['street2'] ?? '' }}<br>
                    {{ $billing['city'] ?? '' }}@if(!empty($billing['state'])), {{ $billing['state'] }}@endif
                    @if(!empty($billing['pincode'])) – {{ $billing['pincode'] }}@endif<br>{{ $billing['country'] ?? '' }}
                  @else
              ...
              <div class="address-block">
                <div style="display:flex;align-items:center;justify-content:space-between;">
                  ...
                  @if($hasShipping)<button onclick="openAddressPanel('shipping');return false;" style="background:none;border:none;cursor:pointer;color:#888;font-size:13px;padding:2px 6px;">✏️</button>@endif
                </div>
                <div class="address-value">
                  @if($hasShipping)
                    {{ $shipping['attention'] ?? '' }}<br>{{ $shipping['street1'] ?? '' }} {{ $shipping['street2'] ?? '' }}<br>
                    {{ $shipping['city'] ?? '' }}@if(!empty($shipping['state'])), {{ $shipping['state'] }}@endif
                    @if(!empty($shipping['pincode'])) – {{ $shipping['pincode'] }}@endif<br>{{ $shipping['country'] ?? '' }}
                  @else
          ...
          <div class="section" id="section-other">
            ...
            <div class="section-body" id="body-other">
              @php $ad = $customer->additional_datas ?? []; @endphp
              <div class="detail-row"><div class="detail-key">Customer Type</div><div class="detail-val">{{ ucfirst($customer->customer_type ?? 'Business') }}</div></div>
              <div class="detail-row"><div class="detail-key">Default Currency</div><div class="detail-val">{{ $ad['currency'] ?? 'INR' }}</div></div>
              <div class="detail-row">
                <div class="detail-key">Portal Status</div>
                <div class="detail-val {{ empty($ad['enable_portal']) ? 'disabled' : '' }}">{{ !empty($ad['enable_portal']) ? 'Enabled' : 'Disabled' }}</div>
              </div>
              <div class="detail-row"><div class="detail-key">Customer Language</div><div class="detail-val">{{ $ad['language'] ?? 'English' }}</div></div>
              @if(!empty($customer->pan))<div class="detail-row"><div class="detail-key">PAN</div><div class="detail-val">{{ $customer->pan }}</div></div>@endif
              @if(!empty($ad['payment_terms']))<div class="detail-row"><div class="detail-key">Payment Terms</div><div class="detail-val">{{ $ad['payment_terms'] }}</div></div>@endif
            </div>
          </div>
          ...
          <div class="section" id="section-contacts">
            ...
            <div class="section-body" id="body-contacts">
              @php $contacts = $ad['contact_persons'] ?? []; @endphp
              <div id="cp-list">
                @if(count($contacts) > 0)
                  @foreach($contacts as $idx => $cp)
                  @php $isPrimary = ($idx === $primaryIdx); @endphp
                  <div class="cp-list-item">
                    <div>
                      <div class="cp-list-name">
                        {{ trim(($cp['salutation'] ?? '').' '.($cp['first_name'] ?? '').' '.($cp['last_name'] ?? '')) }}
                        @if($isPrimary)<span class="primary-badge">★ Primary</span>@endif
                      </div>
                      <div class="cp-list-meta">
                        {{ $cp['email'] ?? '' }}
                        @if(!empty($cp['work_phone'])) &nbsp;·&nbsp; {{ $cp['work_phone'] }} @endif
                        @if(!empty($cp['mobile'])) &nbsp;·&nbsp; {{ $cp['mobile'] }} @endif
                      </div>
                    </div>
                    <div class="cp-gear-wrap">
                      <button class="cp-gear-btn" onclick="toggleRowGear(event, {{ $idx }})">⚙</button>
                      <div class="cp-gear-menu" id="rowGear{{ $idx }}">
                        <div class="cp-gear-menu-item" onclick="openEditCp({{ $idx }}); closeAllGears();"><span>✏️</span> Edit</div>
                        @if(!$isPrimary)
                        <div class="cp-gear-menu-item primary-action" onclick="markPrimary({{ $idx }}); closeAllGears();"><span>★</span> Mark as Primary</div>
                        @endif
                        <div class="cp-gear-menu-sep"></div>
                        <div class="cp-gear-menu-item danger" onclick="openDelConfirm({{ $idx }}); closeAllGears();"><span>🗑️</span> Delete</div>
                      </div>
                    </div>
                  </div>
                  @endforeach
        ...
        <div class="detail-right">
          ...
          <div class="info-value">{{ $ad['payment_terms'] ?? 'Due on Receipt' }}</div>
          ...
          <table class="receivables-table">
            ...
            <tbody>
              <tr>
                <td>{{ ($ad['currency'] ?? 'INR') === 'INR' ? 'INR- Indian Rupee' : ($ad['currency'] ?? 'INR') }}</td>
      ...
      <div id="tab-comments" style="display:none;flex:1;overflow:hidden;">
        <div class="comments-wrap">
          ...
          <div class="comments-section-title">
            ...
            <span class="comments-count-badge" id="commentsCount">{{ count($comments) }}</span>
          </div>
...
<div id="tab-history" style="display:none;flex:1;overflow-y:auto;">
  <div style="padding:24px 28px;max-width:860px;">
    ...
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:20px;">
      ...
      <span style="background:#1a73e8;color:#fff;font-size:11px;font-weight:700;padding:1px 7px;border-radius:10px;">{{ $histories->count() }}</span>
    </div>
    ...
    <div style="display:flex;gap:12px;margin-bottom:18px;">
      ...
      <div style="flex:1;">
        ...
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;flex-wrap:wrap;">
          <span style="font-size:13px;font-weight:600;color:#222;">{{ $h->user->name ?? 'System' }}</span>
          <span style="background:{{ $color }}22;color:{{ $color }};font-size:10px;font-weight:700;padding:2px 8px;border-radius:10px;">{{ $label }}</span>
          <span style="font-size:12px;color:#888;">{{ $h->created_at->setTimezone('Asia/Kolkata')->format('d/m/Y h:i A') }}</span>        </div>
        ...
        <div style="background:#f8f9fc;border:1px solid #e8eaf0;border-radius:6px;padding:10px 14px;font-size:12.5px;">
          @if($h->action === 'create')
            ...
            @if(!empty($h->new_data['display_name']))
              — <strong>{{ $h->new_data['display_name'] }}</strong>
            @endif
          ...
          @else
            {{-- UPDATE: show field-by-field changes --}}
            @php
              $fieldLabels = [
            ...
            @foreach($h->new_data ?? [] as $key => $newVal)
            @php
            ...
            <div style="margin-bottom:6px;padding-bottom:6px;border-bottom:1px solid #f0f0f0;">
              <span style="color:#888;font-size:11px;text-transform:uppercase;letter-spacing:0.4px;">{{ $label2 }}</span><br>
              @if($oldVal !== null)
                <span style="color:#e53935;font-size:12px;">{{ is_bool($oldVal) ? ($oldVal?'Yes':'No') : $oldVal }}</span>
                <span style="color:#888;margin:0 4px;">→</span>
              @endif
              <span style="color:#22c55e;font-size:12px;">{{ is_bool($newVal) ? ($newVal?'Yes':'No') : $newVal }}</span>
            </div>
            @endforeach
          @endif
        </div>
...
var billingData  = @json($customer->common_address['billing']  ?? []);
var shippingData = @json($customer->common_address['shipping'] ?? []);
var customerId   = {{ $customer->id }};
var cpData       = @json($contacts);
var delIdx       = -1;
...
function updateContactCard(pIdx) {
  ...
  if(cpData.length===0||pIdx<0){nameEl.innerHTML='{{ $customer->display_name }}';emailEl.textContent='{{ $customer->email ?? "" }}';return;}
  var cp=cpData[pIdx];
  var name=[cp.salutation,cp.first_name,cp.last_name].filter(Boolean).join(' ').trim();
