    ...
    <div class="content">
      ...
      <div class="page-header">
        ...
        <div class="header-actions">
          {{-- ⚙️ PREFERENCES BUTTON — custom fields configure --}}
        ...
        @if($customers->count() > 0)
          <table class="data-table" id="customersTable">
            ...
            <tbody>
              @foreach($customers as $customer)
                ...
                @endphp
                <tr data-name="{{ strtolower($customer->display_name) }}"
                    data-email="{{ strtolower($customer->email ?? '') }}"
                    data-phone="{{ $customer->phone_number ?? '' }}"
                    data-type="{{ $customer->customer_type }}">

                  <td><input type="checkbox" class="row-check" value="{{ $customer->id }}" style="cursor:pointer;"></td>

                  <td>
                    <div class="cust-name-cell">
                      <div class="cust-avatar" style="background:{{ $color }};">{{ $initials }}</div>
                      <div class="cust-name-info">
                        <span class="cust-name-main">{{ $customer->display_name }}</span>
                        @if($customer->company_name)
                          <span class="cust-name-sub">{{ $customer->company_name }}</span>
                        @endif
                      </div>
                    </div>
                  </td>
                  ...
                  <td>
                    @if($customer->email)
                      <a href="mailto:{{ $customer->email }}" style="color:#3b6cf8;text-decoration:none;">
                        {{ $customer->email }}
                      </a>
                    @else
                  ...
                  <td>
                    <div class="action-wrap">
                      <a href="{{ route('customers.show', $customer->id) }}" class="btn-action" title="View">👁 View</a>
                      <a href="{{ route('customers.edit', $customer->id) }}" class="btn-action" title="Edit">✏ Edit</a>
                      <button type="button" class="btn-action del"
                              onclick="confirmDelete({{ $customer->id }}, '{{ addslashes($customer->display_name) }}')"
                              title="Delete">🗑</button>
                    </div>
                  </td>
                </tr>
          ...
          <div class="pagination-wrap">
            ...
            <div class="pagination-links">
              ...
              @else
                <a href="{{ $customers->previousPageUrl() }}" class="page-link">‹</a>
              @endif
              ...
              @if($customers->hasMorePages())
                <a href="{{ $customers->nextPageUrl() }}" class="page-link">›</a>
              @else
                <span class="page-link disabled">›</span>
              @endif
            </div>
