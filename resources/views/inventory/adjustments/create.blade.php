...
<style>
    ...
    .tag-item .remove-tag { cursor: pointer; color: #888; font-size: 14px; line-height: 1; }
    .tag-field-wrapper input { border: none; outline: none; font-size: 13px; flex: 1; min-width: 80px; }
    .tag-dropdown-btn { cursor: pointer; color: #888; font-size: 18px; padding: 0 2px; }
    ...
    .item-info .item-desc { font-size: 11px; color: #888; margin-top: 2px; }
    ...
    .reporting-tag-btn svg { color: #aaa; }
...
<div class="adj-wrapper">
    <div class="adj-title">New Adjustment</div>

    <form id="adjustmentForm" action="{{ route('inventory.adjustments.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
                ...
                <div class="radio-group">
                    <label>
                        <input type="radio" name="mode" value="quantity" id="modeQty"
                            {{ old('mode', 'quantity') === 'quantity' ? 'checked' : '' }}>
                        Quantity Adjustment
                    </label>
                    <label>
                        <input type="radio" name="mode" value="value" id="modeVal"
                            {{ old('mode') === 'value' ? 'checked' : '' }}>
                        Value Adjustment
                    </label>
                </div>
            </div>
                    ...
                    @foreach($accounts as $group => $accountList)
                        <optgroup label="{{ $group }}">
                            @foreach($accountList as $acc)
                                <option value="{{ $acc }}"
                                    {{ old('account', 'Cost of Goods Sold') === $acc ? 'selected' : '' }}>
                                    {{ $acc }}
                                </option>
                            @endforeach
                        </optgroup>
            ...
            <div class="field">
                <select id="reason" name="reason"
                    ...
                    @foreach($reasons as $r)
                        <option value="{{ $r }}" {{ old('reason', 'Stolen goods') === $r ? 'selected' : '' }}>
                            {{ $r }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="field">
                ...
                @foreach($locations as $loc)
                    <option value="{{ $loc->id }}"
                        ...
                        {{ $loc->location_name }}  {{-- ← location_name, name இல்ல --}}
                    </option>
                @endforeach
            </select>
    ...
    <div class="field">
        <select id="product_rules" name="product_rules"
            ...
            @foreach($productRules as $rule)
                <option value="{{ $rule }}"
                    {{ old('product_rules') === $rule ? 'selected' : '' }}>
                    {{ $rule }}
                </option>
            @endforeach
        </select>
        ...
        <div class="form-footer">
            ...
            <a href="{{ route('inventory.adjustments.index') }}" class="btn-link">Cancel</a>
        </div>
...
@push('scripts')
<script>
// ─── All items from DB (passed from controller) ────────────────────────────
let ALL_ITEMS = {!! json_encode($itemsJson) !!};

// Items already in the table
let tableItems = [];       // [{item, rowIndex}, ...]
let bulkSelected = new Set();  // item ids

let currentMode = document.querySelector('input[name="mode"]:checked').value;

// ─── Mode switch → update table columns ───────────────────────────────────
...
function buildRow(item, rowIndex) {
    ...
    const col4Input = isQty
        ? `<input type="number" step="any" class="qty-input" name="items[${rowIndex}][new_quantity_on_hand]" placeholder="Eg. +10, -10" oninput="calcAdjusted(${rowIndex})">`
        : `<input type="number" step="any" class="qty-input" name="items[${rowIndex}][changed_value]" placeholder="Eg. +10, -10" oninput="calcAdjustedVal(${rowIndex})" value="0.00">`;

    const col5Input = isQty
        ? `<input type="number" step="any" class="qty-input" name="items[${rowIndex}][quantity_adjusted]" placeholder="Eg. +10, -10" oninput="calcNewQty(${rowIndex})">`
        : `<input type="number" step="any" class="qty-input" name="items[${rowIndex}][adjusted_value]" placeholder="Eg. +10, -10" oninput="calcChangedVal(${rowIndex})">`;
...
// Recalculate quantity adjusted when new_qty_on_hand changes
...
function rerenderAllRows() {
    ...
    tableItems.forEach((entry, idx) => {
        tbody.insertAdjacentHTML('beforeend', buildRow(entry.item, idx));
    });
    // Add blank search row at end
    addSearchRow();
}
...
function addSearchRow() {
    ...
    <tr id="searchRow">
        <td></td>
        <td><input class="search-item-input" type="text" placeholder="Type or click to select an item."
            oninput="filterInlineSearch(this.value)" onfocus="showInlineDropdown(this)"></td>
        <td><span class="qty-placeholder">0.00</span></td>
        <td></td>
        <td><span class="qty-placeholder">${isQty ? 'Eg. +10, -10' : 'Eg. +10, -10'}</span></td>
    </tr>
...
function selectInlineItem(itemId) {
    ...
    // Location select பண்ணலன்னா warn பண்ணு
    if (!locationId) {
        alert('Please select a Location first.');
        return;
    }

    // ALL_ITEMS-ல item இருக்கா பாரு
    const item = ALL_ITEMS.find(i => i.id == itemId);
    if (!item) return;

    // Location select ஆகி இருந்தா, fresh stock fetch பண்ணி add பண்ணு
    fetch(`/inventory/adjustments/items-by-location?location_id=${locationId}`)
        .then(r => r.json())
        .then(data => {
            ALL_ITEMS = data;
            // Updated ALL_ITEMS-ல item தேடு
            const freshItem = ALL_ITEMS.find(i => i.id == itemId);
            if (freshItem) addItemToTable(freshItem);
        })
        .catch(() => {
            // Fallback: fetch fail ஆனா existing item use பண்ணு
            addItemToTable(item);
        });
...
function addItemToTable(item) {
    // Remove existing search rows
    ['searchRow','searchRow_reporting','inlineDropdownRow'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.remove();
    });
    const rowIndex = tableItems.length;
    tableItems.push({ item, rowIndex });
    const tbody = document.getElementById('itemRows');
    tbody.insertAdjacentHTML('beforeend', buildRow(item, rowIndex));
    addSearchRow();
}
...
function confirmBulkAdd() {
    // Clear current table items and rebuild from bulkSelected
    tableItems = [];
    ALL_ITEMS.filter(i => bulkSelected.has(i.id)).forEach(item => {
        tableItems.push({ item, rowIndex: tableItems.length });
    });
    rerenderAllRows();
    closeBulkModal();
}

// ─── Product Rules Tag input ───────────────────────────────────────────────
...
function addTagOnEnter(e) {
    if (e.key === 'Enter') {
        ...
        const input   = document.getElementById('productRulesInput');
        const tag = document.createElement('div');
        tag.className = 'tag-item';
        tag.innerHTML = `<span>${escHtml(val)}</span><span class="remove-tag" onclick="removeTag(this)">×</span>`;
...
function syncTagHidden() {
    const tags = [...document.querySelectorAll('#tagWrapper .tag-item span:first-child')]
        .map(s => s.textContent).join(', ');
    document.getElementById('productRulesHidden').value = tags;
}

// ─── Form submit: inject status value ─────────────────────────────────────
document.querySelectorAll('[name="status"]').forEach(btn => {
    btn.addEventListener('click', function () {
        document.querySelectorAll('[name="status"]').forEach(b => b.removeAttribute('form'));
    });
});
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ─── Location change → location-specific stock fetch ──────────────
  // Location change listener-ல loading state add பண்ணுங்கள்
document.getElementById('location_id').addEventListener('change', function () {
    ...
    fetch(`/inventory/adjustments/items-by-location?location_id=${locationId}`)
        ...
        .then(data => {
            ALL_ITEMS = data;
            rerenderAllRows(); // ← fetch complete ஆனா மட்டும் render
        })
        .catch(err => {
