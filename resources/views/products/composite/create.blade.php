{{-- resources/views/products/composite/create.blade.php --}}
@extends('layouts.app')

@section('title', 'New Composite Item')
...
<div class="form-panel">
  ...
  <form id="compositeForm"
        method="POST"
        action="{{ route('composite-items.store') }}"
        enctype="multipart/form-data">
    @csrf

    {{-- Hidden JSON field for associate items --}}
  ...
  {{ old('type', 'assembly_item') === 'assembly_item' ? 'checked' : '' }}
  ...
  onchange="toggleKitFields(this.value)">
        ...
        <div class="form-row">
          ...
          <div class="form-content">
            <input class="form-input" type="text" name="sku"
                   autocomplete="off" value="{{ old('sku') }}">
        ...
        {{-- Category — custom dropdown (same as create.blade.php) --}}
        <div class="form-row">
        ...
        {{-- ↑ LEFT COLUMN ends here — </div> closes flex:1 div --}}
      </div>
      ...
      <div>
        ...
        <div class="col-field">
          ...
          <textarea class="form-input" name="sales_description" rows="3"
                    style="resize:vertical;">{{ old('sales_description') }}</textarea>
        </div>
      </div>
      ...
      <div id="purchase-info-section">
        ...
        <div class="col-field">
          ...
          <textarea class="form-input" name="purchase_description" rows="3"
                    style="resize:vertical;">{{ old('purchase_description') }}</textarea>
        </div>
        <div class="col-field">
...
@endphp
    ...
    <div class="two-col">
      {{-- LEFT: Dimensions, Manufacturer, UPC, EAN --}}
      <div>
        <div class="col-field">
          <div style="color:#555;font-size:13px;margin-bottom:4px;">Dimensions</div>
          <div class="dim-row">
            <input class="dim-input" type="number" step="0.01" min="0"
                   name="custom_field[length]" value="{{ old('custom_field.length') }}" placeholder="L">
            <span class="dim-sep">x</span>
            <input class="dim-input" type="number" step="0.01" min="0"
                   name="custom_field[width]" value="{{ old('custom_field.width') }}" placeholder="W">
            <span class="dim-sep">x</span>
            <input class="dim-input" type="number" step="0.01" min="0"
                   name="custom_field[height]" value="{{ old('custom_field.height') }}" placeholder="H">
          <select class="dim-unit" name="custom_field[dimension_unit]">
  @foreach(['cm','m','in','ft','mm'] as $u)
    <option value="{{ $u }}" 
      {{ old('custom_field.dimension_unit', $dimUnit) === $u ? 'selected' : '' }}>
      {{ $u }}
    </option>
  @endforeach
</select>
        ...
        <div class="col-field" style="margin-top:14px;">
         <div style="color:#555;font-size:13px;margin-bottom:4px;">Weight</div>
          <div class="dim-row">
            <input class="dim-input" type="number" step="0.01" min="0"
                   name="custom_field[weight]" value="{{ old('custom_field.weight') }}" placeholder="0">
          <select class="dim-unit" name="custom_field[weight_unit]">
  @foreach(['kg','g','lb','oz','mg'] as $u)
    <option value="{{ $u }}" 
      {{ old('custom_field.weight_unit', $weightUnit) === $u ? 'selected' : '' }}>
      {{ $u }}
    </option>
  @endforeach
</select>
        ...
        <div class="col-field">
          ...
          <input class="form-input" type="text" name="upc" value="{{ old('upc') }}">
        </div>

        <div class="col-field">
          <div style="color:#555;font-size:13px;margin-bottom:4px;">EAN <span class="info-icon" title="European Article Number">?</span></div>
          <input class="form-input" type="text" name="ean" value="{{ old('ean') }}">
        </div>
      </div>

      {{-- RIGHT: Weight, Brand (with gear), MPN, ISBN --}}
      <div>
        <div class="col-field">

        {{-- Brand — select + gear button (same as create.blade.php) --}}
        <div class="col-field" style="margin-top:14px;">
        ...
        <div class="col-field">
          ...
          <input class="form-input" type="text" name="mpn" value="{{ old('mpn') }}">
        </div>
    ...
    <div class="track-bin" id="track-bin-section">
      <input type="checkbox" id="trackbin" name="bin_location_tracking" value="1"
             {{ old('bin_location_tracking') ? 'checked' : '' }}
    ...
    <div class="inv-two-col" id="inventory-section">
      ...
      <div class="col-field">
        ...
        <input class="form-input" type="number" step="0.01" min="0"
               name="reorder_point" value="{{ old('reorder_point') }}">
      </div>
    </div>
