    ...
    <div class="table-card">
      <div class="table-header">
        ...
        <div style="display:flex; gap:10px;">
            <a href="{{ route('field_customization.index') }}?from=products"
               class="btn-add" style="background:#6c757d;">
                ⚙️ Preferences
            </a>
            <a href="{{ url('/products/create') }}" class="btn-add">
                + New Item
            </a>
        </div>
      </div>
        ...
        <tbody>
          @foreach($products as $product)
            ...
            <tr class="parent-row" data-parent-id="{{ $product->id }}">
                ...
                {{-- ── TYPE BADGE — item_type + type சேர்த்து காட்டுது ── --}}
                <td>
...
<a href="{{ 
  ...
  @if($isComposite)
    <a href="{{ route('composite-items.show', $product->id) }}" 
       class="btn-action" 
    ...
    <button type="submit" class="btn-action btn-delete" onclick="return confirm('Delete this item?')">Delete</button>
            ...
            @if($isVariantParent && count($variants) > 0)
                ...
                <tr class="variant-row variant-parent-{{ $product->id }}" style="display: none;">
                    ...
                    <td>
                        <a href="{{ url('/products/' . $product->id) }}" class="btn-action btn-view">View</a>
                        <a href="{{ url('/products/' . $product->id . '/edit') }}" class="btn-action btn-edit">Edit</a>
                    </td>
                </tr>
                @endforeach
            @endif
      ...
      <div class="empty-state">
        ...
        <a href="{{ url('/products/create') }}" class="btn-add">+ Create New Item</a>
      </div>
      @endif
    </div>
  </div>
