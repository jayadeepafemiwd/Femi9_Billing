...
<head>
  ...
  <title>{{ $displayProductName ?? $product->name }} | Inventory</title>
  <style>
...
<div class="product-list-panel">
  <div class="panel-header">
    ...
    <div class="panel-actions">
      <button class="panel-btn" onclick="window.location='{{ url('/products/create') }}'">+ New</button>
  ...
  <div class="product-list">
    @foreach($products as $p)
      ...
      @if(!$isVariantParent)
        ...
        <div class="product-list-item {{ ($p->id == $product->id && !$selectedVariant) ? 'active' : '' }}" 
             onclick="window.location='{{ url('/products/' . $p->id) }}'">
          <span class="pprice">₹{{ number_format($p->selling_price ?? 0, 2) }}</span>
    ...
    $variantName === $selectedVariantName
                ...
                @endphp
          <div class="product-list-item {{ $isActive ? 'active' : '' }}" 
     style="border-left: 3px solid #e8edff;"
     onclick="window.location='{{ url('/products/' . $p->id) }}?variant={{ urlencode($variantName) }}'">
              <span class="pprice">₹{{ number_format($variantSellingPrice, 2) }}</span>
              <div class="pname" style="font-weight: 500;">
                {{ $p->name }} - {{ $variantName }}
              </div>
              @if($variantSku)
              <div class="psku" style="font-size: 11px;">SKU: {{ $variantSku }}</div>
              @endif
            </div>
          @endforeach
        @endif
...
@endphp
<a href="{{ $editUrl }}" class="btn-edit-icon" title="Edit">✏️</a>

@if($product->item_type === 'item')
    <button class="btn-adjust" onclick="openOpeningStock()">Adjust Stock</button>
@elseif($product->item_type === 'composite_item')
  <button class="btn-adjust" 
            style="background:#7c3aed;"
            onclick="window.location='{{ route('assemblies.create') }}?composite_item_id={{ $product->id }}'">
      🔩 Create Assemblies
    </button>
@endif

  <button class="btn-more">More ▾</button>
  <a href="{{ url('/products') }}" class="btn-close" title="Back">✕</a>
</div>
      </div>
    ...
    <div style="flex:1;min-width:0;">
      ...
      <div style="margin-bottom:24px;">
        ...
        <div class="detail-row">
          ...
          <span class="detail-value">₹{{ number_format($displayCostPrice ?? 0, 2) }}</span>
        </div>
        @if($inventoryAccount)
        <div class="detail-row">
          <span class="detail-label">Purchase Account</span>
          <span class="detail-value">{{ $inventoryAccount }}</span>
        </div>
        @endif
        @if($purchDesc)
        <div class="detail-row">
          <span class="detail-label">Purchase Description</span>
          <span class="detail-value">{{ $purchDesc }}</span>
        </div>
        @endif
      </div>
      ...
      <div style="margin-bottom:24px;">
        ...
        <div class="detail-row">
          ...
          <span class="detail-value">₹{{ number_format($displaySellingPrice ?? 0, 2) }}</span>
        </div>
        @if($salesDesc)
        <div class="detail-row">
          <span class="detail-label">Sales Description</span>
          <span class="detail-value">{{ $salesDesc }}</span>
        </div>
        @endif
      </div>
      ...
      <div style="margin-bottom:24px;">
        ...
        <div class="detail-row">
          ...
          <span class="detail-value">{{ $product->type == 'goods' ? 'Inventory Items' : 'Service' }}</span>
        </div>
        <div class="detail-row">
          <span class="detail-label">SKU</span>
          <span class="detail-value">{{ $displaySku ?? '—' }}</span>
        </div>
        <div class="detail-row">
        ...
        @endif
        @if($product->inventory_valuation_method)
        <div class="detail-row">
          <span class="detail-label">Valuation Method</span>
          <span class="detail-value">{{ $product->inventory_valuation_method }}</span>
        </div>
        @endif
        @if($itemsDesc)
        <div class="detail-row">
          <span class="detail-label">Description</span>
          <span class="detail-value">{{ $itemsDesc }}</span>
        </div>
        @endif
      </div>
      ...
      {{-- ✅ Assembly / Kit Item — Associate Items Section --}}
@if(in_array($product->type, ['assembly_item', 'kit_item']) && !empty($associateItemDetails))
<hr style="border:none;border-top:1px solid #e0e3ea;margin:20px 0;">
<div class="section-title">
{{ $product->type === 'assembly_item' ? 'Assembly Components' : 'Kit Items' }}
</div>
{{-- Items table --}}
@if(!empty($associateItemDetails['items']))
...
<tbody>
@foreach($associateItemDetails['items'] as $item)
<tr>
<td>{{ $item['name'] ?? '—' }}</td>
<td style="color:#888;font-size:12px;">{{ $item['sku'] ?? '—' }}</td>
<td style="text-align:right;">{{ $item['quantity'] ?? 0 }} {{ $item['unit'] ?? '' }}</td>
<td style="text-align:right;">₹{{ number_format($item['selling_price'] ?? 0, 2) }}</td>
<td style="text-align:right;">₹{{ number_format($item['cost_price'] ?? 0, 2) }}</td>
<td style="text-align:right;font-weight:600;">
₹{{ number_format($item['line_total_selling'] ?? 0, 2) }}
</td>
</tr>
@endforeach
</tbody>
...
@endif
{{-- Services table --}}
@if(!empty($associateItemDetails['services']))
<div style="margin-bottom:16px;">
...
<tbody>
@foreach($associateItemDetails['services'] as $svc)
<tr>
<td>{{ $svc['name'] ?? '—' }}</td>
<td style="color:#888;font-size:12px;">{{ $svc['sku'] ?? '—' }}</td>
<td style="text-align:right;">{{ $svc['quantity'] ?? 0 }} {{ $svc['unit'] ?? '' }}</td>
<td style="text-align:right;">₹{{ number_format($svc['selling_price'] ?? 0, 2) }}</td>
<td style="text-align:right;">₹{{ number_format($svc['cost_price'] ?? 0, 2) }}</td>
<td style="text-align:right;font-weight:600;">
₹{{ number_format($svc['line_total_selling'] ?? 0, 2) }}
</td>
</tr>
@endforeach
</tbody>
...
@endif
{{-- Totals --}}
@if(!empty($associateItemDetails['totals']))
...
₹{{ number_format($associateItemDetails['totals']['selling_price'] ?? 0, 2) }}
</strong>
</div>
...
₹{{ number_format($associateItemDetails['totals']['cost_price'] ?? 0, 2) }}
</strong>
</div>
</div>
@endif
@endif {{-- end assembly/kit check --}}

    </div>

    {{-- RIGHT: Stock Summary Panel --}}
    <div style="width:220px;flex-shrink:0;">
      ...
      <div style="background:#fff;border:1px solid #e0e3ea;border-radius:8px;padding:14px 16px;margin-bottom:16px;">
        ...
        <div class="os-value" style="font-size:22px;font-weight:700;color:#1a2340;">
          {{ number_format($product->opening_stock ?? 0, $decRate) }}
        </div>
      </div>
      ...
      <div style="background:#fff;border:1px solid #e0e3ea;border-radius:8px;padding:14px 16px;margin-bottom:16px;">
        ...
        <div style="margin-bottom:10px;">
          ...
          <div style="font-size:15px;font-weight:700;color:#1a2340;">: {{ number_format($totalStockOnHand, 2) }}</div>
        </div>
        <div style="margin-bottom:10px;">
          <div style="font-size:11px;color:#888;">Committed Stock</div>
          <div style="font-size:15px;font-weight:700;color:#1a2340;">: {{ number_format($totalCommitted, 2) }}</div>
        </div>
        <div>
          <div style="font-size:11px;color:#888;">Available for Sale</div>
          <div style="font-size:15px;font-weight:700;color:#1a2340;">: {{ number_format($totalAvailable, 2) }}</div>
        </div>
      </div>
      ...
      <div style="background:#fff;border:1px solid #e0e3ea;border-radius:8px;padding:14px 16px;margin-bottom:16px;">
        ...
        <div style="font-size:16px;font-weight:700;color:#1a2340;">{{ number_format($product->reorder_point, $decRate) }}</div>
      </div>
      @else
      ...
      <span style="color:#333;font-weight:700;">DIRECT SALES ₹0.00</span>
        ...
        <div class="tab-pane" id="tab-locations">
          ...
          <table class="locations-table" id="stock-locations-table">
            ...
            <tbody>
              @forelse($stockLocations as $loc)
              <tr data-loc-id="{{ $loc['id'] }}">
                <td>
                  {{ $loc['location_name'] }}
                  @if($loc['location_type'] === 'business')
                    <span style="color:#f59e0b;" title="Primary">★</span>
                  @endif
                </td>
                <td style="text-align:right;" class="td-stock">{{ number_format($loc['stock_on_hand'], $decRate) }}</td>
                <td style="text-align:right;" class="td-committed">{{ number_format($loc['committed'], $decRate) }}</td>
                <td style="text-align:right;" class="td-available">{{ number_format($loc['available'], $decRate) }}</td>
              </tr>
              @empty
              <tr id="empty-row">
            ...
            <tfoot>
              <tr style="background:#f8fafd;font-weight:600;" id="total-row">
                <td>Total</td>
                <td style="text-align:right;" id="total-stock">{{ number_format(collect($stockLocations)->sum('stock_on_hand'), $decRate) }}</td>
                <td style="text-align:right;" id="total-committed">{{ number_format(collect($stockLocations)->sum('committed'), $decRate) }}</td>
                <td style="text-align:right;" id="total-available">{{ number_format(collect($stockLocations)->sum('available'), $decRate) }}</td>
              </tr>
            </tfoot>
            @endif
          </table>
          ...
          <table class="transactions-table">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>{{ $txn->type }}</td>
                <td>{{ $txn->reference ?? '—' }}</td>
                <td class="{{ $txn->quantity > 0 ? 'txn-in' : 'txn-out' }}">{{ $txn->quantity > 0 ? '+' : '' }}{{ number_format($txn->quantity, 2) }}</td>
                <td>{{ number_format($txn->balance ?? 0, 2) }}</td>
              </tr>
              @endforeach
            </tbody>
          </table>
...
<div class="tab-pane" id="tab-history">
  ...
  <div class="history-list">
    ...
    <div class="history-item">
      <div class="history-dot" style="background: {{ $h->action == 'create' ? '#10b981' : ($h->action == 'delete' ? '#ef4444' : '#2d5be3') }}"></div>
      <div style="flex:1;">

        {{-- Action badge --}}
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
          <span style="
            background: {{ $h->action == 'create' ? '#d1fae5' : ($h->action == 'delete' ? '#fee2e2' : '#eef2ff') }};
            color: {{ $h->action == 'create' ? '#065f46' : ($h->action == 'delete' ? '#991b1b' : '#3730a3') }};
            font-size:11px; font-weight:700; padding:2px 8px; border-radius:4px; text-transform:uppercase;
          ">{{ $h->action }}</span>
          <span style="font-size:12px;color:#888;">{{ $h->created_at->format('d M Y, h:i A') }}</span>
          <span style="font-size:12px;color:#555;font-weight:600;">by {{ $h->user->name ?? 'System' }}</span>
        </div>
        ...
        {{-- Update action — changed fields show pannurom --}}
    ...
    @if(in_array($field, ['updated_at', 'created_at', 'deleted_at'])) @continue @endif
              <tr style="border-bottom:1px solid #f0f2f7;">
                <td style="padding:5px 10px;color:#555;font-weight:500;">{{ $field }}</td>
...
<table style="font-size:12px;border-collapse:collapse;width:100%;max-width:500px;">
  ...
  <tbody>
    @foreach($h->new_data['locations'] as $i => $loc)
    @php $oldLoc = $h->old_data['locations'][$i] ?? null; @endphp
    <tr style="border-bottom:1px solid #f0f2f7;">
      <td style="padding:5px 10px;color:#555;">{{ $loc['location'] }}</td>
      <td style="padding:5px 10px;color:#ef4444;text-align:right;">
        {{ $oldLoc ? number_format($oldLoc['stock'], 2) : '0.00' }}
      </td>
      <td style="padding:5px 10px;color:#10b981;text-align:right;font-weight:600;">
        {{ number_format($loc['stock'], 2) }}
      </td>
    </tr>
    @endforeach
  </tbody>
  <tfoot>
    <tr style="background:#f8fafd;">
      ...
      <td style="padding:5px 10px;color:#ef4444;font-weight:700;text-align:right;">
        {{ number_format($h->old_data['total_stock'] ?? 0, 2) }}
      </td>
      <td style="padding:5px 10px;color:#2d5be3;font-weight:700;text-align:right;">
        {{ number_format($h->new_data['total_stock'], 2) }}
      </td>
    </tr>
  </tfoot>
</table>
...
<div id="osModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;justify-content:center;align-items:flex-start;padding-top:60px;">
  <div style="background:#fff;border-radius:8px;width:820px;max-height:80vh;display:flex;flex-direction:column;box-shadow:0 8px 32px rgba(0,0,0,0.2);">
    <div style="padding:14px 24px;border-bottom:1px solid #e0e3ea;display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">
      <span style="font-size:16px;font-weight:700;color:#1a2340;">{{ $product->name }}</span>
...
<script>
const _locs    = @json($stockLocations ?? []);
const _pid     = {{ $product->id }};
const _decRate = {{ $decRate }};
const _wStep   = "{{ $wStep }}";
let   _n       = 0;
...
function selectVariant(productId, variantName, variantSku, variantPrice) {
    ...
    fetch('{{ url("/products/set-variant") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
...
function clearVariantSelection() {
    ...
    fetch('{{ url("/products/clear-variant") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
