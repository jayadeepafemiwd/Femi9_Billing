...
@section('title', isset($transferOrder) ? 'Edit Transfer Order #' . $transferOrder->transfer_order_number : 'New Transfer Order')
...
.page-header .status-badge {
    ...
    border-radius: 20px; text-transform: capitalize; margin-left: 6px;
}
.status-draft       { background: #f0f0f0; color: #666; }
.status-initiated   { background: #fff3cd; color: #856404; }
.status-transferred { background: #d1f0e0; color: #155724; }
...
.btn-primary-group { display: flex; align-items: stretch; border-radius: 4px; overflow: visible; position: relative; }
...
.btn-primary-group .btn-main-solo { padding: 7px 16px; background: #2d7dd2; color: #fff; border: none; font-size: 13px; font-weight: 500; cursor: pointer; border-radius: 4px; }
.btn-primary-group .btn-main-solo:hover { background: #246bb8; }
...
.split-dropdown { display: none; position: absolute; bottom: 110%; right: 0; background: #fff; border: 1px solid #d0d0d0; border-radius: 4px; box-shadow: 0 4px 16px rgba(0,0,0,0.12); min-width: 200px; z-index: 100; }
.split-opt { padding: 9px 16px; cursor: pointer; font-size: 13px; color: #333; border-bottom: 1px solid #f0f0f0; }
.split-opt:last-child { border-bottom: none; }
.split-opt:hover { background: #eaf2fb; }
.split-opt.primary { color: #2d7dd2; font-weight: 600; }
...
.alert-warning { background: #fff8e1; border: 1px solid #ffe082; border-radius: 4px; padding: 10px 14px; margin-bottom: 14px; color: #856404; font-size: 13px; }
.error-msg { color: #e74c3c; font-size: 11px; margin-top: 3px; }

.same-location-warning { display: none; color: #e74c3c; font-size: 12px; margin-top: 8px; padding: 6px 10px; background: #fdf0ee; border-radius: 4px; border: 1px solid #f5c6cb; }
</style>
@endpush
...
<div class="page-header">
    ...
    <h2>
        {{ isset($transferOrder) ? 'Edit Transfer Order' : 'New Transfer Order' }}
        @isset($transferOrder)
            <span class="status-badge status-{{ $transferOrder->status }}">
                {{ ucfirst($transferOrder->status) }}
            </span>
        @endisset
    </h2>
...
<div class="form-container">
    ...
    @isset($transferOrder)
        @if($transferOrder->status === 'transferred')
            <div class="alert-warning">
                ⚠️ This Transfer Order has already been transferred. Only data will be updated — stock will not be affected
            </div>
        @endif
    @endisset
    ...
    <form id="toForm" method="POST"
        ...
        <div class="same-location-warning" id="sameLocationWarning">
            ⚠️ Source and Destination location same-ஆ இருக்கு! Transfer முடியாது.
        </div>
                            ...
                            <td>
                                <div class="item-detail-cell">
                                    ...
                                    <div>
                                        ...
                                        @if($item->variant_id)
                                            @php
                                                $variant = \DB::table('item_variants')->find($item->variant_id);
                                            @endphp
                                            <span class="item-variant-badge">{{ $variant->name ?? '' }}</span>
                                        @endif
                                    </div>
                                </div>
        ...
        <div class="form-footer">
            ...
            {{-- Save as Draft — Transferred இல்லாதபோது மட்டும் --}}
            @if(!isset($transferOrder) || $transferOrder->status !== 'transferred')
                <button type="button" class="btn-secondary" onclick="submitDraft()">Save as Draft</button>
            @endif

            <div class="btn-primary-group">

                @if(!isset($transferOrder))
                    {{-- ✅ CREATE PAGE --}}
                    <button type="submit" name="action" value="initiate" class="btn-main">Initiate Transfer</button>
                    <button type="button" class="btn-split" onclick="toggleSplitDrop()">&#9660;</button>
                    <div class="split-dropdown" id="splitDropdown">
                        <div class="split-opt primary" onclick="submitAction('mark_as_transfer')">
                            &#8644; Mark as Transfer
                        </div>
                ...
                @elseif($transferOrder->status === 'initiated')
                    {{-- ✅ EDIT — INITIATED --}}
                    <button type="submit" name="action" value="initiate" class="btn-main">Update</button>
                    <button type="button" class="btn-split" onclick="toggleSplitDrop()">&#9660;</button>
                    <div class="split-dropdown" id="splitDropdown">
                        <div class="split-opt primary" onclick="submitAction('mark_as_transfer')">
                            &#8644; Mark as Transfer
                        ...
                        <div class="split-opt" onclick="submitAction('initiate')">
                            &#9654; Update & Keep Initiated
                        </div>
                    </div>

                @elseif($transferOrder->status === 'transferred')
                    {{-- ✅ EDIT — TRANSFERRED: Update மட்டும் --}}
                    <button type="submit" name="action" value="transferred" class="btn-main-solo">Update</button>
        ...
        <div class="bulk-modal-body">
            ...
            <div class="bulk-right">
                <div class="bulk-right-title">
                    ...
                    <span class="bulk-total">Total: <span id="bulkTotalQty">0</span></span>
                </div>
...
function checkSameLocation() {
    const srcVal = document.getElementById('source_location_id').value;
    const dstVal = document.getElementById('destination_location_id').value;
...
function renderItemDropdown(q) {
    ...
    ALL_PRODUCTS.forEach(p => {
        ...
        if (p.has_variants && p.variants && p.variants.length) {
            p.variants.forEach(v => {
                ...
                if (matchP || matchV) {
                    ...
                    html += `<div class="item-opt item-opt-selectable"
                        ...
                        data-name="${eh(p.name)}" data-vname="${eh(v.name)}"
                        data-sku="${eh(v.sku||p.sku||'')}" data-unit="${eh(p.unit||'box')}"
            ...
            count++;
            html += `<div class="item-opt item-opt-selectable"
                data-pid="${p.id}" data-vid=""
                data-name="${eh(p.name)}" data-vname=""
                data-sku="${eh(p.sku||'')}" data-unit="${eh(p.unit||'box')}"
                data-image="${eh(p.image_url||'')}">
                <span class="item-opt-name">${eh(p.name)}</span>
    ...
    dd.innerHTML = count ? html : '<div class="item-opt" style="color:#aaa;cursor:default;">No items found</div>';
}

function focusSearch() { document.getElementById('itemSearchInput').focus(); }

// ── Stock AJAX ────────────────────────────────────────────────────────────────

function fetchStock(pid, vid, srcId, dstId, cb) {
    if (!pid || !srcId || !dstId) { cb(0, 0); return; }
    const params = new URLSearchParams({ product_id: pid, source_location_id: srcId, destination_location_id: dstId });
...
function removeRow(btn) { btn.closest('tr').remove(); reindex(); }
...
function filterBulkItems(q) { renderBulkList(q); }
...
function renderBulkList(q) {
    ...
    ALL_PRODUCTS.forEach(p => {
        ...
        if (p.has_variants && p.variants && p.variants.length) {
            p.variants.forEach(v => {
                ...
                if (matchP || matchV) {
                    ...
                    html += `<div class="bulk-item-row${isChk?' selected':''} bulk-item-selectable"
                        data-key="${key}" data-pid="${p.id}" data-vid="${v.id}"
                        data-name="${eh(p.name)}" data-vname="${eh(v.name)}"
                        data-sku="${eh(v.sku||p.sku||'')}" data-unit="${eh(p.unit||'box')}"
                        data-image="${eh(p.image_url||'')}">
            ...
            const isChk = !!bulkSelected[key];
            html += `<div class="bulk-item-row${isChk?' selected':''} bulk-item-selectable"
                data-key="${key}" data-pid="${p.id}" data-vid=""
                data-name="${eh(p.name)}" data-vname=""
                data-sku="${eh(p.sku||'')}" data-unit="${eh(p.unit||'box')}"
                data-image="${eh(p.image_url||'')}">
                <input type="checkbox" class="bulk-check" ${isChk?'checked':''} tabindex="-1">
...
function renderBulkSelected() {
    ...
    if (!keys.length) {
        document.getElementById('bulkSelectedList').innerHTML = '<div class="bulk-empty">Click the item names from the left pane to select them</div>';
        return;
    }
...
function submitAction(action) {
    ...
    const dstVal = document.getElementById('destination_location_id').value;
    if (srcVal && dstVal && srcVal === dstVal) {
        alert('⚠️ Source and Destination location same-ஆ இருக்கு! Transfer முடியாது.');
        return;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('bulkSelectedList').addEventListener('click', function(e) {
        ...
        if (!btn) return;
        delete bulkSelected[btn.dataset.key];
        renderBulkList(document.getElementById('bulkSearch').value);
        renderBulkSelected();
    });

    // Form submit — same location check
    document.getElementById('toForm').addEventListener('submit', function(e) {
        const srcVal = document.getElementById('source_location_id').value;
        const dstVal = document.getElementById('destination_location_id').value;
        if (srcVal && dstVal && srcVal === dstVal) {
            e.preventDefault();
