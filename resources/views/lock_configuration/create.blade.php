...
<div class="container-fluid py-4">
    <div class="card shadow-sm" style="max-width: 780px; margin: auto;">
        ...
        <div class="card-body py-4 px-4">
            ...
            <form action="{{ route('lock_configuration.store') }}" method="POST" id="lockForm">
                @csrf
                ...
                <div class="row mb-4 align-items-start">
                    ...
                    <div class="col-sm-8">
                        <textarea name="description"
                                  class="form-control"
                                  rows="3"
                                  placeholder="testing">{{ old('description') }}</textarea>
                    </div>
                </div>
                ...
                <div class="row mb-3 align-items-start">
                    ...
                    <div class="col-sm-8">
                        {{-- Main dropdown (matches Zoho: Restrict All / Restrict Selected / Allow Selected) --}}
                        <select name="action_type" id="actionTypeSelect" class="form-select mb-2">
                            <option value="restrict_all"      {{ old('action_type') == 'restrict_all'      ? 'selected' : '' }}>Restrict All Actions</option>
                            <option value="restrict_selected" {{ old('action_type', 'restrict_selected') == 'restrict_selected' ? 'selected' : '' }}>Restrict Selected Actions</option>
                            <option value="allow_selected"    {{ old('action_type') == 'allow_selected'    ? 'selected' : '' }}>Allow Selected Actions</option>
                        </select>

                        {{-- Actions sub-dropdown (Edit / Delete) - hidden when restrict_all --}}
                        <div id="actionsSubSection">
                            <select id="actionsDropdownTrigger" class="form-select mb-1" style="display:none">
                                {{-- Just a visual trigger, actual checkboxes below --}}
                            </select>
                            <div id="actionsCheckboxList" class="border rounded p-2 bg-white">
                                ...
                                <div class="form-check py-1 border-bottom">
                                    <input class="form-check-input" type="checkbox"
                                           ...
                                           id="actionEdit"
                                           {{ in_array('Edit', old('selected_actions', [])) ? 'checked' : '' }}>
                                ...
                                <div class="form-check py-1">
                                    <input class="form-check-input" type="checkbox"
                                           ...
                                           id="actionDelete"
                                           {{ in_array('Delete', old('selected_actions', [])) ? 'checked' : '' }}>
                ...
                <div class="row mb-3 align-items-start">
                    ...
                    <div class="col-sm-8">
                        ...
                        <select name="field_type" id="fieldTypeSelect" class="form-select mb-2">
                            <option value="restrict_all"      {{ old('field_type') == 'restrict_all'      ? 'selected' : '' }}>Restrict All Fields</option>
                            <option value="restrict_selected" {{ old('field_type') == 'restrict_selected' ? 'selected' : '' }}>Restrict Selected Fields</option>
                            <option value="allow_selected"    {{ old('field_type', 'allow_selected') == 'allow_selected' ? 'selected' : '' }}>Allow Selected Fields</option>
                        </select>

                        {{-- Fields list (hidden when restrict_all) --}}
                                    ...
                                    @foreach($availableFields as $key => $label)
                                        <div class="form-check px-3 py-1 field-item" data-label="{{ strtolower($label) }}">
                                            <input class="form-check-input" type="checkbox"
                                                   ...
                                                   value="{{ $key }}"
                                                   id="field_{{ $key }}"
                                                   {{ in_array($key, old('selected_fields', [])) ? 'checked' : '' }}>
                ...
                <div class="row mb-4 align-items-start">
                    ...
                    <div class="col-sm-8">
                        <select name="lock_for_type" id="lockForTypeSelect" class="form-select mb-2">
                            <option value="all_roles"        {{ old('lock_for_type', 'all_roles') == 'all_roles'        ? 'selected' : '' }}>All Roles</option>
                            <option value="all_roles_except" {{ old('lock_for_type') == 'all_roles_except'              ? 'selected' : '' }}>All Roles Except</option>
                            <option value="selected_roles"   {{ old('lock_for_type') == 'selected_roles'                ? 'selected' : '' }}>Selected Roles</option>
                        </select>

                        {{-- Roles list (shown only when all_roles_except or selected_roles) --}}
