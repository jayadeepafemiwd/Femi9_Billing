...
<div class="container-fluid py-4">
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-white d-flex justify-content-between align-items-center py-3">
            ...
            <a href="{{ route('lock_configuration.create') }}" class="btn btn-primary btn-sm">
                    ...
                    <table class="table table-hover mb-0 align-middle">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                {{-- Actions --}}
                                <td>
                                    <span class="badge bg-secondary">{{ $config->action_type_label }}</span>
                                    @if($config->selected_actions && count($config->selected_actions))
                                        <br>
                                        @foreach($config->selected_actions as $action)
                                            <span class="badge bg-warning text-dark mt-1">{{ $action }}</span>
                                        @endforeach
                                    @endif
                                </td>
                                ...
                                {{-- Fields --}}
                                <td>
                                    <span class="badge bg-secondary">{{ $config->field_type_label }}</span>
                                    @if($config->selected_fields && count($config->selected_fields))
                                        <br>
                                        @foreach(array_slice($config->selected_fields, 0, 3) as $field)
                                            <span class="badge bg-info text-dark mt-1">
                                                {{ $availableFields[$field] ?? $field }}
                                            </span>
                                        @endforeach
                                        @if(count($config->selected_fields) > 3)
                                            <span class="badge bg-light text-dark mt-1">
                                                +{{ count($config->selected_fields) - 3 }} more
                                            </span>
                                        @endif
                                    @endif
                                </td>
                                ...
                                {{-- Lock For --}}
                                <td>
                                    <span class="badge bg-secondary">{{ $config->lock_for_type_label }}</span>
                                    @if($config->roles && count($config->roles))
                                        <br>
                                ...
                                {{-- Status --}}
                                <td>
                                    <form action="{{ route('lock_configuration.toggle', $config->id) }}"
                                          method="POST" class="d-inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
                                            class="badge border-0 {{ $config->status === 'active' ? 'bg-success' : 'bg-danger' }}"
                                ...
                                {{-- Actions --}}
                                <td>
                                    <a href="{{ route('lock_configuration.edit', $config->id) }}"
