{{-- resources/views/assign_location/create.blade.php --}}
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
function currentLayer() {
    ...
    if (depth === 0) {
        return allLayers.find(l => l.parent_id === null && !l.is_global) || null;
    }

    const lastId = String(navPath[depth - 1].itemId);

    // ✅ FIX: must match BOTH parent_id AND depth
    const specific = allLayers.find(
        l => !l.is_global 
          && l.parent_id === lastId 
          && Number(l.depth) === depth   // ← ADD THIS LINE
    );
    if (specific) return specific;

    const globalLayer = allLayers.find(
        l => l.is_global && Number(l.depth) === depth
    );
    return globalLayer || null;
}
/* ─────────────────────────────────────────────────────────────
   KEY FIX: Returns true if a layer already exists at the
   current navigation depth + parent item, so we can block
   "+ Add Layer" from being triggered again.
────────────────────────────────────────────────────────────── */
function layerAlreadyExistsHere() {
    if (navPath.length === 0) {
        // Root level: a non-global layer with parent_id=null already exists
        return allLayers.some(l => l.parent_id === null && !l.is_global);
    }

    const lastId = String(navPath[navPath.length - 1].itemId);
    const depth  = navPath.length;

    // A specific layer already exists under this exact parent item
    const hasSpecific = allLayers.some(l => !l.is_global && l.parent_id === lastId);
    if (hasSpecific) return true;

    // A global layer already exists at this depth
    const hasGlobal = allLayers.some(
        l => l.is_global && l.parent_id === `__GLOBAL__${depth}`
    );
    return hasGlobal;
}

/* ── Get parent layer type (for checkbox label) ── */
function getParentLayerType() {
    if (navPath.length === 0) return null;

    const lastId = String(navPath[navPath.length - 1].itemId);

    for (const layer of allLayers) {
        const found = (layer.names || []).some(n => String(n.id) === lastId);
        if (found) return layer.layer_type;
    }

    for (const val of globalValues) {
        const found = (val.names || []).some(n => String(n.id) === lastId);
        if (found) {
            const gl = allLayers.find(l => String(l.id) === String(val.layer_id));
            return gl ? gl.layer_type : null;
        }
    }
...
/* ── Get all sibling item names (for global hint) ── */
function getSiblingNames() {
    if (navPath.length === 0) return [];

    if (navPath.length === 1) {
        const rootLayer = allLayers.find(l => l.parent_id === null && !l.is_global);
        return rootLayer ? (rootLayer.names || []).map(n => n.name) : [];
    }

    const parentItemId = String(navPath[navPath.length - 2].itemId);

    for (const layer of allLayers) {
        const found = (layer.names || []).some(n => String(n.id) === parentItemId);
        if (found) return (layer.names || []).map(n => n.name);
    }

    for (const val of globalValues) {
        const found = (val.names || []).some(n => String(n.id) === parentItemId);
        if (found) return (val.names || []).map(n => n.name);
    }
...
function drillInto(itemId, itemName) {
    navPath.push({ itemId: String(itemId), itemName });
    cancelForms();
}
...
function handleMainAdd() {
    ...
    if (!layer) {
        // No layer here — allow creating one
        addingLayer = true;
        addingItem  = false;
    } else {
        // Layer exists — add items to it
        addingItem  = true;
        addingLayer = false;
    }
...
function focusActiveInput() {
    const el = document.getElementById('layerTypeInp') || document.getElementById('tagRealInp');
    if (el) el.focus();
}
...
/* ── Save layer type ── */
async function saveLayerType() {
    const val      = (document.getElementById('layerTypeInp')?.value || '').trim();
    const chk      = document.getElementById('globalChk');
    const isGlobal = chk ? chk.checked : false;

    if (!val) { alert('Please enter a layer type name!'); return; }

    // Frontend guard: block if a specific layer already exists here
    if (!isGlobal && layerAlreadyExistsHere()) {
        alert('A layer already exists under this item. Delete it first before creating a new one.');
        return;
    }

    const parentItemId = navPath.length > 0
        ? String(navPath[navPath.length - 1].itemId)
        : null;

    const depth = navPath.length;

    const res = await api('/assign-location/add-layer-type', {
        layer_type : val,
        parent_id  : isGlobal ? null : parentItemId,
        is_global  : isGlobal,
        depth      : depth,
    });
...
function addPendingTag(name) {
    ...
    if (!clean) return;
    const layer = currentLayer();
    const saved = currentLayerNames(layer).map(n => n.name.toLowerCase());
...
async function deleteItem(layerId, nameId) {
    ...
    const layer = currentLayer();
    const parentNameId = layer && layer.is_global && navPath.length > 0
        ? String(navPath[navPath.length - 1].itemId)
        : null;

    if (navPath.length > 0 && String(navPath[navPath.length - 1].itemId) === String(nameId)) {
        navPath.pop();
    }

    const payload = { layer_id: layerId, name_id: nameId };
    if (parentNameId) payload.parent_name_id = parentNameId;

    await api('/assign-location/delete-name', payload);
    await loadData();
}
...
function renderBreadcrumb() {
    ...
    let html = `<span class="bc-item ${navPath.length === 0 ? 'active' : ''}"
                      onclick="navigateTo(0)">Assign Location</span>`;
    navPath.forEach((step, i) => {
        const isLast = i === navPath.length - 1;
        html += `<span class="bc-sep">›</span>
                 <span class="bc-item ${isLast ? 'active' : ''}"
                       onclick="navigateTo(${i + 1})">${esc(step.itemName)}</span>`;
    });
    bc.innerHTML = html;
}
...
async function handleDeleteLayer() {
    ...
    const isGlobal     = layer.is_global;
    const parentNameId = isGlobal && navPath.length > 0
        ? String(navPath[navPath.length - 1].itemId)
        : null;

    let msg;
    if (isGlobal && parentNameId) {
        const itemName = navPath[navPath.length - 1].itemName;
        msg = `Delete "${cap(layer.layer_type)}" layer for "${itemName}" only?\n\nAll values under ${itemName} will be deleted.`;
    } else if (isGlobal) {
        msg = `Delete the GLOBAL "${layer.layer_type}" layer?\n\nThis removes it for ALL items at this level!`;
    } else {
        msg = `Delete the "${layer.layer_type}" layer?\n\nAll names inside will be permanently deleted.`;
    }
...
function renderTopButton() {
    ...
    if (layer) {
        // Layer exists → button label shows what we're adding (items)
        addBtn.textContent = `+ Add ${cap(layer.layer_type)}`;
        addBtn.disabled    = false;
    } else if (alreadyHas) {
        // Edge case: layer exists in allLayers but currentLayer() missed it
        addBtn.textContent = '+ Add Layer';
        addBtn.disabled    = true;
    } else {
        // No layer at all → allow creating one
        addBtn.textContent = '+ Add Layer';
        addBtn.disabled    = false;
    }
    ...
    if (layer) {
        ...
        delBtn.textContent = `🗑 Delete "${cap(layer.layer_type)}" Layer`;
...
function renderCenter() {
    ...
    const layer = currentLayer();
    const names = currentLayerNames(layer);

    let title, hint;
    if (navPath.length === 0) {
        title = layer ? cap(layer.layer_type) : 'Assign Location';
        hint  = layer
            ? `Click any ${layer.layer_type} to go deeper`
            : 'Start by adding a layer (e.g. Country)';
    } else {
        title = navPath[navPath.length - 1].itemName;
        hint  = layer
            ? (layer.is_global
                ? `🌐 This ${layer.layer_type} layer applies to all — click to drill deeper`
                : `Select a ${layer.layer_type} to drill deeper`)
    ...
    let chipsHtml = '';
    if (layer) {
        chipsHtml = names.map(item => {
            const sid  = String(item.id);
            const snam = item.name.replace(/'/g, "\\'");
            return `<div class="item-chip" onclick="drillInto('${sid}','${snam}')">
                        <span>${esc(item.name)}</span>
                        <span class="chip-arrow">›</span>
                        <span class="chip-del"
                              onclick="event.stopPropagation();deleteItem('${layer.id}','${sid}')">✕</span>
                    </div>`;
        }).join('');
    }
    ...
    if (addingLayer) {
        const parentType   = getParentLayerType();
        ...
        <div class="center-form">
            ...
            <input type="text" id="layerTypeInp" class="simple-inp"
                   placeholder="Layer type name..." />

            ${showGlobal ? `
            <label class="global-check-wrap" for="globalChk">
                <input type="checkbox" id="globalChk"
                       onchange="toggleGlobalHint(this)">
                <span class="global-check-label">
                    Apply to all <span>${esc(cap(parentType || 'items'))}</span>
                </span>
            </label>
            <div class="global-hint-box" id="globalHintBox">
                ✅ This layer will be <strong>shared</strong> across:
                <strong>${esc(siblingLabel)}</strong>
            </div>
            ` : ''}

            <div class="form-actions">
                <button class="btn btn-success btn-sm" onclick="saveLayerType()">Save Layer</button>
                <button class="btn btn-outline btn-sm" onclick="cancelForms()">Cancel</button>
            </div>
        </div>`;
    }

    else if (addingItem && layer) {
        ...
        formHtml = `
        <div class="center-form">
            <div class="center-form-title">Add ${cap(layer.layer_type)} names</div>
            <div class="center-form-hint">
                Type a name and press <strong>Space</strong>, <strong>Comma</strong>, or <strong>Enter</strong> to add.<br>
                Add as many as you want, then click Save.
            </div>
            <div class="tag-box" onclick="document.getElementById('tagRealInp').focus()">
                ${pendHtml}
                <input type="text" id="tagRealInp" class="tag-real-inp"
                       placeholder="${pendingTags.length === 0 ? 'Type ' + layer.layer_type + ' name...' : 'Add more...'}"
                       onkeydown="onTagKeydown(event)"
                       oninput="onTagInput(event)" />
            </div>
            <div class="form-actions">
    ...
    if (!layer && !addingLayer) {
        emptyHtml = `<div class="empty-state">
            <div class="icon">🌍</div>
            <p>No layer defined here yet.<br>
               Click <strong>"+ Add Layer"</strong> to get started.</p>
        </div>`;
    } else if (layer && names.length === 0 && !addingItem) {
        emptyHtml = `<div class="empty-state">
            <div class="icon">📍</div>
            <p>No ${esc(layer.layer_type)}s added yet.<br>
               Click <strong>"+ Add ${esc(cap(layer.layer_type))}"</strong> to add one.</p>
        </div>`;
    }
    ...
    if (addingLayer) setTimeout(() => document.getElementById('layerTypeInp')?.focus(), 60);
    if (addingItem)  setTimeout(() => document.getElementById('tagRealInp')?.focus(), 60);
}
...
document.addEventListener('keydown', e => {
    ...
    if (e.key === 'Enter' && addingLayer) {
        if (document.activeElement?.id === 'layerTypeInp') saveLayerType();
    }
});
</script>
</body>
